...
- 手动直连：支持手动输入 IPv4 或 `[IPv6]:端口`，并一键复制当前设备可直连的 IPv6 地址
- 双向回传修复：桌面端现在同时监听 IPv4 / IPv6，请求到达时会回写最新源地址，避免“能收到却回不去”的旧地址缓存问题
...
请确保手机和电脑连接在同一局域网。Windows 安装器会为 `EchoText.exe` 自动添加覆盖 Private 与 Public 网络的入站规则，并允许 EchoText 接收来自真实校园网 / 热点 / 家庭网环境中的 IPv4 与 IPv6 连接；如果你从源码运行，首次弹出防火墙提示时仍需允许本地网络访问。
...
复制 IPv6 按钮现在参考 `program_sharer_pc_stable.py` 的取址方式：会综合主机名解析结果和本机网络接口扫描结果来挑选 `AF_INET6` 地址，自动去掉 zone id，并过滤 `::1` 与 `fe80::` 链路本地地址，避免把不能跨设备直连的地址复制出来。
...
- 设备互相发现不到：
  确认两台设备在同一 Wi-Fi，且网络没有屏蔽本地广播。升级后请把两端都重新打开一次，新版本会同时向 `255.255.255.255`、多个本地子网广播地址发送发现包，并在历史地址之间自动回退。像校园网这种 `/17`、`/16` 的大网段，现在也会按真实子网掩码计算广播地址，不再误判成 `/24`。
- 校园网下发现不到设备：
  很多校园网会把同一 SSID 下的终端拆到不同子网，或者直接开启 client isolation / AP isolation，导致广播发现和终端互访都被限制。这种情况下不是 EchoText 单独能完全绕过去的问题。新版本已经补上真实子网广播、多地址广播和历史地址回退；如果像 `ping 对方 IP` 仍然直接报“无法访问目标主机”，通常说明网络层已经禁止终端互访，只能改用宿舍局域网、手机热点，或联系网络管理员确认策略。
- 配对失败：
...
- Android 能接收 Windows 消息，但无法回发：
  旧版本里常见原因是 Windows 端只监听 IPv4，或者手机还缓存着 Windows 的旧地址。最新版本已经补上桌面端双栈监听，并在收到消息时自动把对端当前源地址写回缓存。升级两端后重新打开一次应用；如果仍失败，再手动直连一次或重新配对，把旧热点 / 旧 Wi-Fi 地址刷掉。
- 复制出的 IPv6 直连不上 / Android 端显示没有可分享的 IPv6 地址：
  Android 端已修复 IPv6 地址探测：除了主机名解析，现在还会直接扫描网络接口上的全部 IPv6 地址（与 IPv4 候选逻辑保持一致），不再仅依赖主机名返回。如果仍然无法直连，通常是当前局域网没有真正提供可互访的 IPv6 路由。
- Windows 桌面端中文仍显示异常：
