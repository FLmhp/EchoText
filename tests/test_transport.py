...
def test_pair_and_send_message_over_ipv6_loopback() -> None:
    if not socket.has_ipv6:
        pytest.skip("IPv6 is not available on this machine")

    receiver_identity = DeviceIdentity("receiver", "Receiver", "Windows", "::1", 0)
    sender_identity = DeviceIdentity("sender", "Sender", "Android", "::1", 9999)
    pair_code = PairCode()
    paired_peers: dict[str, Peer] = {}
    received: list[TextMessage] = []
    ...
    def on_message(message: TextMessage, peer: Peer) -> None:
        received.append(message)
        paired_peers[peer.device_id] = peer
    ...
    try:
        ...
        pytest.skip("Dual-stack IPv6 binding is not available on this machine")
    server.start()
    try:
        client = TransportClient()
        target = Peer("receiver", "Receiver", "Windows", "::1", server.port)
        secret = generate_shared_secret()
        client.pair(target, sender_identity, pair_code.code, secret)
        paired_target = Peer("receiver", "Receiver", "Windows", "::1", server.port, shared_secret=secret)
        message = TextMessage("message-id", "sender", "Sender", "hello-ipv6", 1.0)

        client.send_message(paired_target, message)

        assert paired_peers["sender"].host == "::1"
        assert received == [message]
    finally:
        server.stop()
